...
func TestTriggerSuccessCase(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(res http.ResponseWriter, req *http.Request) {
		...
		assert.Equal(t, lib, req.Header["X-Channel-Library"][0])
		assert.NoError(t, err)
	}))
	defer server.Close()
...
func TestInitialisationFromURL(t *testing.T) {
	url := "http://feaf18a411d3cb9216ee:fec81108d90e1898e17a@api.webpubsub.com/apps/104060"
	client, _ := ClientFromURL(url)
	expectedClient := &Client{Key: "feaf18a411d3cb9216ee", Secret: "fec81108d90e1898e17a", AppID: "104060", Host: "api.webpubsub.growingbox.cn"}
	assert.Equal(t, expectedClient, client)
}

func TestURLInitErrorNoSecret(t *testing.T) {
	url := "http://fec81108d90e1898e17a@api.webpubsub.growingbox.cn/apps"
...
func TestURLInitHTTPS(t *testing.T) {
	url := "https://key:secret@api.webpubsub.growingbox.cn/apps/104060"
	client, _ := ClientFromURL(url)
...
func TestInitialisationFromENV(t *testing.T) {
	os.Setenv("CHANNEL_URL", "http://feaf18a411d3cb9216ee:fec81108d90e1898e17a@api.webpubsub.growingbox.cn/apps/104060")
	client, _ := ClientFromEnv("CHANNEL_URL")
	expectedClient := &Client{Key: "feaf18a411d3cb9216ee", Secret: "fec81108d90e1898e17a", AppID: "104060", Host: "api.webpubsub.growingbox.cn"}
	assert.Equal(t, expectedClient, client)
}
