...
var channelPathRegex = regexp.MustCompile("^/apps/([0-9]+)$")
var maxTriggerableChannels = 100

const (
	libraryVersion = "5.0.0"
	libraryName    = "channel-http-go"
)

/*
Client to the HTTP API of Webchan.

There easiest way to configure the library is by creating a `Webchan` instance:

	client := webpubsub.Client{
		AppID: "your_app_id",
		Key: "your_app_key",
...
Changing the `webpubsub.Client`'s `Host` property will make sure requests are sent
to your specified host.

	client.Host = "foo.bar.com" // by default this is "api.webpubsub.growingbox.cn".
*/
type Client struct {
	AppID                        string
	Key                          string
	...
	OverrideMaxMessagePayloadKB  int     // set the agreed Channel message limit increase
	validatedEncryptionMasterKey *[]byte // parsed key for use
}

/*
ClientFromURL allows client instantiation from a specially-crafted Webchan URL.

	c := webpubsub.ClientFromURL("http://key:secret@api.webpubsub.growingbox.cn/apps/app_id")
...
func ClientFromURL(serverURL string) (*Client, error) {
	...
	matches := channelPathRegex.FindStringSubmatch(url2.Path)
	if len(matches) == 0 {
...
This is particularly relevant if you are using Channel as a Heroku add-on,
which stores credentials in a `"WEBCHAN_URL"` environment variable. For example:
	client := webpubsub.ClientFromEnv("WEBCHAN_URL")
...
/*
Trigger triggers an event to the Channel API.
...
type TriggerParams struct {
	...
	// http://webpubsub.com/docs/duplicates.
	SocketID *string
	...
	// contact us at http://support.webpubsub.com if you wish to enable this.
	// Pass in `nil` if you do not wish to specify any query attributes.
	// This is part of an [experimental feature](https://webpubsub.com/docs/lab#experimental-program).
	Info *string
}
...
https://webpubsub.com/docs/channels/library_auth_reference/rest-api#request
for a complete list.
...
You can read more here: http://webpubsub.com/docs/duplicates.
...
type Event struct {
	...
	Data     interface{}
	SocketID *string
	// Info is part of an [experimental feature](https://webpubsub.com/docs/lab#experimental-program).
	Info *string
}
...
TriggerBatch triggers multiple events on multiple channels in a single call:
	...
	socketID := "1234.12"
	client.TriggerBatch([]webpubsub.Event{
...
In this library, one does this by passing a `webpubsub.MemberData` instance.
...
/*
Webhook allows you to check that a Webhook you receive is indeed from Channel, by
checking the token and authentication signature in the header of the request. On
your dashboard at http://app.webpubsub.com, you can set up webhooks to POST a
...
https://webpubsub.com/docs/webhooks.



If the webhook is valid, a `*webpubsub.Webhook* will be returned, and the `err`
...
func (c *Client) Webhook(header http.Header, body []byte) (*Webhook, error) {
	for _, token := range header["X-Channel-Key"] {
		if token == c.Key && checkSignature(header.Get("X-Channel-Signature"), c.Secret, body) {
